...
'    MaxCol = Range("A1").End(xlToRight).Column
    DiffRow = InputBox("連続データを比較したい開始行を入力してください(例:2)")
    MaxRow = Cells(Rows.Count, 1).End(xlUp).Row
    MaxCol = Cells(DiffRow, Columns.Count).End(xlToLeft).Column

'    DiffCol = "L"
    DiffCol = InputBox("連続データを比較したい列を入力してください(例:L)")
    'DiffCol2 = "I"
    
    '// 選択行数ループ
    For iRow = 0 + (DiffRow - 1) To MaxRow - 1
